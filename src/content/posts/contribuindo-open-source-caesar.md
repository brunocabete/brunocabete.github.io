---
title: "Contribuindo para projetos open source: Julius e Augustus"
author: Bruno Cabete
pubDatetime: 2026-06-18T18:00:00Z
slug: contribuindo-para-projetos-os-julius-augustus
featured: true
draft: false
tags:
  - Open Source
...
Ao baixar a versão de linux do jogo, vi que a interface original ficava um pouco pequena no meu monitor moderno, então ampliei a escala de interface para 200% só pra ver que agora o meu mouse não clicava mais onde eu queria. Para conseguir clicar em um botão, eu precisava ficar escaneando o vazio abaixo e a direita dele lentamente até ver o efeito de hover e saber que era la que o jogo achava que meu cursor estava.

Foi aí que minha curiosidade começou a trabalhar. Quem sabe seja um bug simples? Como é open source, quem sabe consigo até corrigir?

Fui lá no repositório do Augustus, pesquisei nas issues, e... Achei alguém com o mesmo bug que eu! E além disso tinha outra pessoa dando uma solução que aparentemente iria resolver o problema! Será que a galera não deu muita atenção porque a solução estava perdido em um comentário na issue e nenhuma pull request tinha sido aberta? Será que consigo abrir uma PR para os mantenedores aceitarem e então resolver um problema que impede a galera do linux de jogar?
...
O culpado parecia ser o tal do SDL, biblioteca usada em praticamente todas as plataformas, do PC ao PS Vita, pra que os devs possam interagir com a parte de áudio, vídeo e input do hardware de forma transparente e independente da plataforma (nesse caso transparente significa abstraído, e não exposto. Me incomoda um pouco que a mesma palavra tem sentidos praticamente opostos).
...
Depois de fuçar um pouco nas pastas de build automatizada do Github actions e tentar replicar os processos com docker, ferramentas locais e um pouco de alquimia digital para gerar build de diferentes plataformas, não tive muito sucesso, mas consegui algumas pistas importantes:
...
E assim fui testando. Algumas versões logo depois da 2.0.20, algumas mais recentes, algumas mais pro meio, e nunca que eu achava uma que quebrava o jogo. Cadê a versão 2.32.68 que eu sei que quebra? Não existe? De onde minha distro tirou isso? **SDL2-compat**???
...
Saabendo disso, resolvi ver qual pacote o flatpak usava e, logicamente, ele usava a versão compat, e por isso o bug.
...
Os caras lançaram o patch enquanto eu escrevia esse texto!

Conclusão: Fui tentar jogar Caesar 3 e saí sabendo mais sobre C e suas ferramentas de compilação, bibliotecas dinâmicas e como são carregadas em runtime, desenvolvimento de jogos, ações automatizadas do Github e como funcionam os modos de distribuição de software no Linux! Mal posso esperar para encontar outro bug.